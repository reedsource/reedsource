...
public class InjectRedis {
	...
	/**
	 * 获取一个基于redis的永久自增长id,本id请不要在任何时间手动修改数据
	 */
	public String getRedisId() {
		return getRedisGrowId("topId");
	}

	/**
	 * @return 基于当天时间+redis的id
	 */
	public String getRedisTimeId() {
		return DealDate.get8Time() + getRedisId();
	}

	/**
	 * 获取一个基于redis的自增长以天为单位循环的id
	 *
	 * @return
	 */
	public String getRedisDayId() {
		String key = "topDayId";
		String value = getRedisGrowId(key);
		//redis set会重置时间,本处重新设置结束时间
		expireAt(value, getDayEndDate());
		return value;
	}

	/**
	 * @return 获取一个基于redis的自增长id, 主要用于维护, 维护数据前先删除本key, 然后将获得从1开始的新的id
	 */
	public String getRedisAutoId() {
		return getRedisGrowId("WHId");
	}

	/**
	 * 清空维护id  实际赋值为0
	 */
	public void emptyRedisAutoId() {
		set("WHId", "0");
	}
	...
	/**
	 * @param key 自增长id key
	 * @return 基于redis的自增长id
	 */
	public String getRedisGrowId(String key) {
		String a = TopConstant.S_1;
		if (Boolean.TRUE.equals(hasKey(key))) {
			a = String.valueOf(Integer.valueOf(get(key)) + 1);
		}
		set(key, a);
		return a;
	}
