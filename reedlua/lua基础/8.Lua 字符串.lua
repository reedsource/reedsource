...
print("字符串全部转为大写字母")
print(string.upper("reed"))

print("字符串全部转为小写字母")
print(string.lower("REED"))

print("在字符串中替换")
print("string.gsub(mainString,findString,replaceString,num)在字符串中替换。")
print("mainString 为要操作的字符串， findString 为被替换的字符，replaceString 要替换的字符，num 替换次数（可以忽略，则全部替换），如")
print(string.gsub("reed", "e", "a", 1))

print("在一个指定的目标字符串 str 中搜索指定的内容")
print("string.find (str, substr, [init, [end]]) substr，如果找到了一个匹配的子串，就会返回这个子串的起始索引和结束索引，不存在则返回 nil。")
print(string.find("Hello Lua user", "Lua", 1))

print("字符串反转")
print(string.reverse("Lua"))

print("类似printf的格式化字符串")
print(string.format("the value is:%d", 4))
-- the value is:4

print("将整型数字转成字符并连接")
print("string.char(arg) 和 string.byte(arg[,int])  byte 转换字符为整数值(可以指定某个字符，默认第一个字符) ")
...
print("寻找源字串str中的第N个配对")
print("参数init可选, 指定搜寻过程的起点, 默认为1。")
print("在成功配对时, 函数将返回配对表达式中的所有捕获结果; 如果没有设置捕获标记, 则返回整个配对字符串. 当没有成功的配对时, 返回nil")
...
--无符号整数  2022年6月6日 预期报错
--不可为-数
--目前的编译器一般int为32位的,所以-1用%u输出为4294967295,如果你是这样写的unsigned a=-1;其实是两步,第一步：unsigned a；
--第二步：a=-1；因为a为unsigned型的,所以-1会先强制转换为unsigned型,
-- -1在内存中是1111111111111111111111111111111共32个1,因为按%u读取,所以为2^32-1=4294967295
